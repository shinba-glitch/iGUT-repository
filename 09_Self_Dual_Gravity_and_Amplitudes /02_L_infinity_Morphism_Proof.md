# 02. L_infinity-Morphism とエネルギースケールの創発（厳密証明）
...
## 2. iGUT の状態空間と結合則の定義
- **状態空間:** $\mathcal{V}_{\text{red}} \cong C^\infty(\mathbb{C}P^1, \mathbb{R})$ （$\mathbb{C}P^1$ 上のスカラー関数＝情報流の揺らぎ）
...
Lemma 1 より、両者のブラケットの間には $C(z, \bar{z}) = \frac{2}{(1+|z|^2)^2}$ という空間依存の共形因子が生じる。
これは「曲がった内部幾何 $\mathbb{C}P^1$」と「平坦な運動学空間 $\mathbb{R}^2$」の幾何学的なズレである。iGUT から観測可能なマクロ時空への射影を行う関手 $F_P$ は、この幾何学的な曲率因子を、SDGにおける「粒子の運動学的なエネルギースケール $\omega$」として吸収（レンダリング）する役割を果たす。
$$\text{Internal Curvature } (1+|z|^2)^2 \xrightarrow{F_P} \text{Kinematic Scale } \omega$$

### Theorem 1: $F_P$ 射影を介した $L_\infty$-Morphism の成立
関手 $F_P$ によるスケール因子の吸収を伴う写像 $\tilde{\Phi}_1 \equiv F_P \circ \Phi_1$ を定義する。この写像の下で、曲率因子は相殺・規格化され、以下の方程式が恒等的に満たされる。
